...
def integral_img(im):
    ...
    a = np.cumsum(im, axis=1)  # na desno
    b = np.cumsum(a, axis=0)  # na dole
...
# 3.4
def mutual_information(x, y, nbins=10):

    h2d, xedges, yedges = np.histogram2d(np.reshape(x, -1), np.reshape(y,-1), bins=nbins)

    pxy = h2d/float(np.sum(h2d))  # normalizovani 2D histogram - zdruzena verovatnoca x i y
    px = np.sum(pxy, axis=1)  # marginalna raspodela po x
    py = np.sum(pxy, axis=0)  # marginalna raspodela po y
    px_py = np.outer(px,py)

    nzp = pxy>0  # lokacije gde histogram nije prazan
    mi = np.sum(pxy[nzp]*np.log(pxy[nzp]/px_py[nzp]))

    # entropije slike x i y, potrebno za normalizovane zajednicke informacije
...
# 3.6
def normalized_mutual_information(x, y, nbins=10):

    mi, hx, hy = mutual_information(x, y, nbins)
    nmi = 1-(mi/np.maximum(hx, hy))
